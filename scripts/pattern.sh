...
#SBATCH --output=results/pattern_logs/slurm_%j.out
...
for i in 0; do
  seed=$(($RAND_SEED + i))
  python main.py --cfg configs/GRIT/pattern-GRIT-RRWP.yaml accelerator "cuda:$i" optim.max_epoch "$N_EPOCH" seed "$seed" &
done
wait
...
if [ "$NEXT_EPOCH" -gt 105 ]; then
    echo "Fin des 100 epochs"
    exit 0
fi
...
sbatch --export=ALL scripts/pattern.sh
