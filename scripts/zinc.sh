...
#SBATCH --output=results/zinc-GRIT-RRWP/log/slurm_%j.out
...
for i in 0; do
  seed=$(($RAND_SEED + i))
  python main.py --cfg configs/GRIT/zinc-GRIT-RRWP.yaml accelerator "cuda:$i" optim.max_epoch "$N_EPOCH" seed "$seed" &
done
wait
