...
#SBATCH --output=results/cifar-10_logs/slurm_%j.out
...
for i in 0; do
  seed=$(($RAND_SEED + i))
  python main.py --cfg configs/GRIT/cifar10-GRIT-RRWP.yaml accelerator "cuda:$i" optim.max_epoch "$N_EPOCH" seed "$seed" &
done
wait
...
sbatch --export=ALL scripts/cifar10.sh
