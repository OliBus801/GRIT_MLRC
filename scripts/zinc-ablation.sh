...
#SBATCH --output=results/zinc-ablations-GRIT-RRWP/log/slurm_%j.out
...
echo "Launching experiment '$CONFIG' with EPOCH_GOAL=$EPOCHS and RAND_SEED=$SEED_BASE"

# On s'assure de nettoyer le param input et de créer le directory pour les logs
rm -f scripts/params/ablations/$CONFIG.input
mkdir -p results/zinc-ablations-GRIT-RRWP/$CONFIG/slogs

# Run 4 training runs with different seeds on 4 GPUs
for i in {0..3}; do
  seed=$(($SEED_BASE + $i))
  echo "python main.py --cfg configs/GRIT/ablations/zinc-GRIT-$CONFIG.yaml accelerator cuda:0 train.ckpt_period $DELTA_EPOCH optim.max_epoch $EPOCHS seed $seed" >> scripts/params/ablations/$CONFIG.input
done

# Lancement parallèle de 4 tâches, chacune sur un GPU distinct
cat scripts/params/ablations/$CONFIG.input | parallel -j4 --lb 'CUDA_VISIBLE_DEVICES=$(({%} - 1)) bash -c {} > results/zinc-ablations-GRIT-RRWP/$CONFIG/slogs/slog_{#}.out 2>&1'
...
sbatch --export=ALL scripts/zinc-ablation.sh