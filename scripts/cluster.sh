...
#SBATCH --output=results/cluster_logs/slurm_%j.out
...
for i in 0 1 2 3; do
  seed=$(($RAND_SEED + i))
  python main.py --cfg configs/GRIT/cluster-GRIT-RRWP.yaml accelerator "cuda:$i" optim.max_epoch "$N_EPOCH" seed "$seed" &
done
wait
...
sbatch --export=ALL scripts/cluster.sh
